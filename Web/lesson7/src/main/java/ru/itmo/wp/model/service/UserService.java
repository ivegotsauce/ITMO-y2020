...
public class UserService {
    ...
    public void validateSetAdmin(long id, long userId, String value) throws ValidationException {
        User current = userRepository.find(id);
        User user = userRepository.find(userId);
        if (user == null) {
            throw new ValidationException("No such user");
        }
        if (!current.getAdmin()) {
            throw new ValidationException("User should be an admin");
        }
        if (!value.equals("enable") && !value.equals("disable")) {
            throw new ValidationException("Invalid admin set request");
        }
    }

    public void setAdmin(long id, String value) {
