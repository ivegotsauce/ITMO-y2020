...
public class UserRepositoryImpl implements UserRepository {
    ...
    public void setAdmin(long id, String value) {
        User user = find(id);
        boolean newAdmin = value.equals("enable");
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE User SET admin=? WHERE id=?")) {
                statement.setBoolean(1, newAdmin);
                statement.setLong(2, user.getId());
                statement.executeQuery();
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't set admin in User", e);
        }
    }
    ...
    public void save(User user, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `User` (`login`, `passwordSha`, `creationTime`, `email`) VALUES (?, ?, NOW(), ?)", Statement.RETURN_GENERATED_KEYS)) {
                ...
                statement.setString(2, passwordSha);
                statement.setString(3, user.getLogin() + "@gmail.com");
                if (statement.executeUpdate() != 1) {
                    ...
                    if (generatedKeys.next()) {
                        ...
                        user.setAdmin(find(user.getId()).getAdmin());
                    } else {
